...
CSIRT:コンピュータセキュリティインシデントに対応するため、企業内での専門チーム

主な活動は以下通り

インシデント発生前：脆弱性情報の収集よりパッチ適用などの予防策
...
JPCERT/CC:CSIRTコーディネートセンター、分析センター

CERT/CC:USの国際連携CSIRT

APCERT:AsiaPacificのコンピュータセキュリティインシデント対応組織
...
OODAとはObeservやOrient、Deciide、Actの頭文字。
...
SOAR企業の中にサイバー攻撃を監視する専任組織としてSOCを設置することがある

監視技術や対応の自動化を進める体制や技術、ソリューションをSOARと呼ぶ

SIEM / SOARと表記されることもある。


### EDR

エンドポイントセキュリティのEDRが近年のランサムに対応するために重視されるようになった。

全てのエンドポイントをまとめてリアルタイムに監視、解析して、攻撃の兆候を検知する


### インシデント管理システム

インシデントの対応状況や進捗状況を管理するシステムのこと。


### デジタルフォレンジックス

インシデント発生時の証拠の取集、状況の再現、情報分析を可能とする技術の総称。

インシデント発生時のスナップショットの保存や各種ログの分析など。

コンピュータ犯罪に対して、犯人の特定、裁判での利用に使える。
...
情報収集：統合管理システムの利用、JVNの情報対策ツール等の利用

深刻度評価：入手した脆弱性に応じて、深刻度を評価する

措置の実施：深刻度評価の結果に従い、対応が必要かを判断する

事象分析：自社の状態を把握しておき、資産管理、監視、脆弱性診断、ペネトレ

### JVN

脆弱性対策の情報ポータルサイトのこと。

ここまでに発見されたソフトウェアなどの脆弱性関連情報や脆弱性情報を収集するツール。

### CVSS

共通脆弱性評価システム。

情報システムの脆弱性に対するオープンで汎用ていな評価手法。
...
環境評価基準：環境＝利用環境も含めて最終的な脆弱性の深刻度を評価

...
その後、IEEE802.11iの仕様が固まった段階で、WPAもIEEE802.11iの機能を取り込む方向に進んだ。→出来上がったのがWPA2
...
大前提として、クラウドサービスに移行しても、自社のセキュリティポリシに合致できるかどうかの確認は不可欠。

その中でもクラウドサービス特有のリスクを加味して、次のような点を重点的に意識する。

**信頼性**

自社のSLAやBCPと比較して、クラウドサービス事業者の障害対策や災害対策が十分なものかどうかをチェックする。

この時に後述するデータセンターの評価基準やガイドラインを使ってよい。

**入退室管理**

部外者が侵入できないような厳重な入退室管理がおこなわれているかも重要なチェックポイント。

情報漏洩や内部不正への対応策と合わせてチェックしておくべき

**クラウドサービス上のデータ**

契約約款を確認して、データ消失の場合の対応をチェックする。

過失の有無にもよるが、クラウド事業者では責任が持てないケースもある。

**その他**

サービスの拡張性やマネージドサービスに関しても自分の要求に合致するかを確認しておくことが望まれる。

契約後の事業環境の変化を予測した上で行う必要があるので、中長期計画との整合性が必要となる部分。

利用者からの問い合わせ窓口はすぐに連絡がつくかどうかなどの、障害発生時を想定して連絡が確実にとれることやFAQの有無などは確認が必要。

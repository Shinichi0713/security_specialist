...
IPネットワーク上にある機器が、自分のIPアドレスのTCPやUDPの特定のポート番号への通信を、指定のアドレスの特定のポートへ自動に転送する機能。

ポートフォワーディングを設定されたポートは、対応づけられた別のアドレス・ポートの窓口として機能し、外部との間で送受信されるパケットは全て自動的に転送される。
SSHの機能を用いることで暗号化された通信経路を用意することを特にSSHポートフォワーディングと呼ぶ。

## パストラバーサルとディレクトリトラバーサルの違いは何？

パストラバーサルとディレクトリトラバーサルは、実際には同じ攻撃手法を指します。どちらも、コンピュータシステムの脆弱性を利用して、本来アクセスできないはずのファイルやディレクトリに不正にアクセスする攻撃

具体的には、攻撃者が特殊な文字列を使用して、プログラムが想定していないファイルやディレクトリへのアクセスを試みます。例えば、ファイル名のパラメータに「../」などを含めることで、上位ディレクトリに移動し、機密情報を取得することができます
...
フロントエンドは「見える部分」、バックエンドは「裏で動いている部分」

フロントエンド：これはウェブサイトやアプリの画面部分。ユーザーが目にするボタンやデザイン、入力フォームなんかがここに含まれるよ。HTML、CSS、JavaScriptなどで作られてるんだ。

バックエンド：こちらは「裏方さん」。ユーザーがデータを保存したり取り出したりするのを処理するサーバー側のシステムだよ。データベースとのやり取りやロジックを管理してるよ。PythonやPHP、Javaなんかがよく使われてる！
...
Webサイトでよく問題になるのが  **XSS（クロスサイト・スクリプティング）攻撃** 。

攻撃者が悪意ある `<script>` をページに埋め込むと、ユーザのCookieや入力情報が盗まれる危険があります。

CSPは「どこからスクリプトや画像を読み込んで良いか」をホワイトリスト方式で指定することで、
...
1. **リソースの読み込み制御**
   * 例: スクリプトは自サイトからのみ、画像はCDNからも許可、など
2. **インラインスクリプトや `eval()`の禁止**
   * 動的コード実行を防ぐ
3. **違反の検知（レポート機能）**

...
入力された項目を使ってSQLを組み立てて攻撃を実行する。
入力チェックが甘い場合に意図しないSQLが実行される。

結果、管理者PWが漏洩したり、変更されてアクセスされることが起きる。
...
各言語での対策は同じ
1. サニタイジング(特殊文字に対してエスケープ処理)
2. バインド機構を利用

## バッファオーバーフロー
C言語やC++にはstrcpyなどの確保しているメモリエリア(バッファ)の大きさを無視した書込みが可能という脆弱性が存在する。
関数を使用してプログラムされたアプリケーションがバッファオーバフローの存在するアプリケーションとなる。

スタックバッファオーバーフロー：
スタックバッファオーバーフローはスタック領域にあるバッファが溢れることを意味する。
...
「⁠確保したバッファの大きさ以上に、データが書き込めてしまうことの、いったい何が問題なのか？」と疑問に思った方もいるかと思います。これは簡単に言えば、図2に示すように、もしプログラムの正常な続行に必要な「大事なデータ」がバッファの付近に存在した場合、上書きできてしまうのが問題なのです。

![alt text](image.png)

攻撃手法
ヒープバッファオーバーフロー攻撃は、隣接するメモリチャンクの管理情報を上書きすることで、任意のメモリ書き換えを可能にします。
これにより攻撃者はプログラムの制御を奪い、悪意ある操作を実行することが出来る。

スタックバッファオーバーフローとヒープバッファオーバーフローは、どちらも**メモリ領域を超えてデータを書き込むことで発生するバッファオーバーフロー**ですが、**発生する場所や影響、典型的な攻撃手法**に違いがあります。
...
### 場所
- **スタック領域**（関数のローカル変数や戻りアドレスなどが格納される）

### 発生例
```c
void func() {
    char buf[10];
    gets(buf); // bufのサイズ以上に入力するとオーバーフロー
}
```

### 影響
- **戻りアドレスやフレームポインタなど、重要な制御情報を上書きできる**
- これにより、任意のコード実行や不正な関数呼び出しが可能
- 典型的な攻撃：**シェルコード実行、リターンアドレス書き換え**

### 攻撃難易度
- 比較的古典的で有名な手法
- 現在は多くのOSでスタック保護機構（例：Stack Canary, DEP, ASLRなど）が導入されている
...
### 場所
- **ヒープ領域**（動的に確保するメモリ領域、malloc/newなどで確保）

### 発生例
```c
char *buf = malloc(10);
strcpy(buf, long_input); // bufのサイズ以上にコピーするとオーバーフロー
```

### 影響
- **ヒープ管理情報や隣接するヒープ領域のデータを上書きできる**
- これにより、ヒープ構造体の破壊や任意データの改ざん、関数ポインタの書き換えなどが可能
- 典型的な攻撃：**ヒープ管理情報の改ざん、Use-After-Free、ダブルフリーなどと組み合わせた攻撃**

### 攻撃難易度
- スタックよりも複雑で、ヒープの実装依存度が高い
- しかし、ヒープ領域の保護が不十分な場合は深刻な被害につながる

---

## まとめ

| 項目                    | スタックバッファオーバーフロー | ヒープバッファオーバーフロー |
|-------------------------|-------------------------------|-----------------------------|
| 発生場所                | スタック領域                  | ヒープ領域                  |
| 主なターゲット          | 戻りアドレス、局所変数など     | ヒープ管理情報、動的変数    |
| 典型的な攻撃            | リターンアドレス改ざん         | ヒープ構造破壊、関数ポインタ改ざん |
| 攻撃難易度・特徴        | 古典的、保護機構が多い         | 複雑、実装依存度高い         |

両者とも**「本来書き込んではいけない領域にデータが書き込まれる」**点は同じですが、**発生場所と攻撃の手法・影響範囲が異なります**。

## CSRF

クロスサイトリクエストフォージュリは攻撃者が用意した悪意あるサイトにアクセスしたことで、スクリプト月のWEBページをダウンロードしてしまい、自動実行され、他サイトにアクションを起こされる攻撃。

CSRF（Cross-Site Request Forgery、クロスサイトリクエストフォージェリ）は、**ユーザーが意図しないリクエストを、ユーザーの権限でWebアプリケーションに送信させる攻撃手法**です。

---

## 仕組みと発生の流れ

1. **被害者があるWebサービス（例：銀行やSNSなど）にログインし、セッションが有効な状態になる**
   - 例：ブラウザにログイン済みで、認証クッキーが保存されている

2. **攻撃者が悪意のあるWebページやメールを用意する**
   - 例：攻撃者のページに「画像タグ」や「フォーム」など、被害サイトへのリクエストを勝手に送る仕掛けを埋め込む

3. **被害者が攻撃者の用意したページを閲覧する**
   - 例：メール内のリンクをクリック、掲示板の投稿を閲覧、広告を表示など

4. **被害者のブラウザが、知らないうちに被害サイトにリクエストを送信する**
   - 例：`<img src="https://bank.example.com/transfer?to=attacker&amount=10000">`
   - このとき、**被害者の認証情報（クッキーなど）が自動的に付与される**

5. **被害サイトは、正規ユーザーからのリクエストと誤認し、処理を実行してしまう**
   - 例：お金の送金、パスワード変更、投稿など

---

## なぜ成立するのか？

- **Webブラウザは、同じドメインへのリクエストには自動的にクッキーなどの認証情報を付ける**
- サーバー側がリクエストの「正当性」を確認する仕組み（CSRFトークンなど）がない場合、攻撃が成立する
...
```
このページを見るだけで、被害者のブラウザは銀行サイトにリクエストを送信し、送金処理が行われる可能性があります。

---

## 防御策

- **CSRFトークン**（正規フォームにしか入っていないランダムな値でリクエストを識別）
- **Refererチェック**（リファラヘッダで出所を確認）
- **SameSite Cookie属性**（クロスサイトからのクッキー送信を制限）

---

## まとめ

CSRFは、**ユーザーが意図しないリクエストを、ユーザーの認証情報を用いてWebアプリケーションに送らせる攻撃**であり、**外部サイト経由でリクエストが送信されることで発生します**。

なので意図しない攻撃を、とあるサイトにアクセスするだけで実行されてしまう攻撃

__対策__
1. cookieとは別のトークンを利用する

トークン＝疑似乱数のこと。
ページトークンとも呼ばれる。

cookieは基本同じページにアクセスするときにブラウザがじどうてきに付与する→CSRFを防御できない。
webページ側でhiddenフィールドに別途トークンを用意しておき、連続性のある政党なリクエストかをチェックしておく

2. 確定時に入力を要求
処理のアック邸するときに改めて入力を要求することも非常に有効。
CAPTCHAなどの手法がある。
...
CISが策定したシステムを安全に構成するための構成基準。
windowsなどの具体的なOS、MW、アプリケーションごｔにセキュリティ対策をまとめている。

### OWASP ASVS

OWASPのプロジェクトの一つ。
webアプリケーションの設計、開発、脆弱性死んだにおいて必要となるセキュリティ要件の標準を確立することを目標として標準を作成、管理している。

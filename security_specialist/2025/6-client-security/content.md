...
- 共通鍵を用いてメッセージと鍵から生成される短い認証コード

ディジタル署名

- 公開鍵暗号方式を利用
- 送信者が秘密鍵で署名を生成、受信者は公開鍵で検証。

### 利用シーン・特徴

MAC

通信路の両端で同じ鍵を持つ場合（例：VPN、TLSの一部）
鍵が漏れると誰でもMACを生成できる
否認防止（non-repudiation）はできない
ディジタル署名

公開鍵を配布できる環境（例：電子メール署名、電子契約、ソフトウェア配布）
署名者の特定が可能、後から「自分はやっていない」と主張できない（否認防止）
第三者が検証可能


## EAP（Extensible Authentication Protocol）の使いどころ

ネットワークのユーザー認証に使われるプロトコル。

Wi-FIやVPNなどの安全な接続を確立する場面で利用される。

認証そのものの仕組みを拡張可能な形でやり取りする共通フレームワーク。

自身では特定の認証方式を定めず格納容器として機能する。

実際の認証方法はEAP-TLSやEAP-PEAPのようなEAPの系列プロトコルが担当する。
...
特に **企業・大学などのセキュアなWi-Fiネットワーク（例：eduroam）** で使われます。

* IEEE 802.1X という仕組みと組み合わせて使われ、

  → 接続時にユーザ名・証明書などを使って本人確認を行います。

例：

* **EAP-TLS** ：電子証明書を使って相互認証（高セキュリティ）
* **EAP-PEAP / EAP-TTLS** ：TLSトンネル内でパスワードをやり取り（よく使われる）

---

### ② VPN接続

リモートアクセスVPNやモバイルVPNでも、

**ユーザ認証プロトコルとしてEAPを利用**する場合があります。

→ PPP（Point-to-Point Protocol）と組み合わせて使われることが多いです。

---

### ③ 有線LANの認証（802.1X）

無線LANと同様に、**有線LANのポート接続時にもEAP認証**が利用されます。

→ PCをLANポートにつないでも、認証が通らないと通信が許可されない仕組み。


## PPP（ **Point-to-Point Protocol** ）

**2つの機器を1対1（point-to-point）で接続し、データ通信を行うための通信プロトコル**です。

主に **インターネット接続（特に昔のダイヤルアップやPPP over Ethernetなど）** に使われてきました。
...
* OSI参照モデルでいうと **データリンク層（Layer 2）** のプロトコルです。
* **電話回線・シリアル通信・モデム・トンネル接続（VPNなど）** のように、

  直接的に2つの機器をつなぐ場面で使われます。


## 📦 PPPの主な役割

PPPは単なるデータ転送ではなく、以下のような複数の機能を提供します。

| 機能                           | 内容                                                                                                                         |
| ------------------------------ | ---------------------------------------------------------------------------------------------------------------------------- |
| **フレーミング**         | データを「フレーム」という単位に区切って送受信できるようにする                                                               |
| **誤り検出**             | CRC（巡回冗長検査）で伝送エラーを検出                                                                                        |
| **認証**                 | PAP（Password Authentication Protocol）や CHAP（Challenge Handshake Authentication Protocol）、またはEAPを使ってユーザを認証 |
| **ネットワーク層の設定** | LCP（Link Control Protocol）や NCP（Network Control Protocol）でIPアドレスなどを自動的に割り当て                             |
...
PPPが電話回線上で動作して、認証・IPアドレス割り当てなどを行っていました。

→ 「PPP接続に失敗しました」というエラーを昔見た人も多いですね。

### ② PPPoE（PPP over Ethernet）

現在でも **光回線などのインターネット接続** で広く使われています。

* PPPをEthernet上でカプセル化して使う方式。
* 認証（ID・パスワード）とセッション確立をEthernet上で実現。
* 日本の多くのISPでは、この **PPPoE** が使われています。

### ③ PPP over Serial / VPN

VPNや組込み通信などでも、シリアル経由でPPPを使うことがあります。

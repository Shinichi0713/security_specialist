...
TKIP（Temporal Key Integrity Protocol）とは、無線LAN（Wi-Fi）のセキュリティ規格である **WPA** において採用された、通信を暗号化するための仕組みです。

ざっくり言うと、 **「脆弱性が指摘された古い暗号化方式（WEP）を、ハードウェアを買い替えずにソフトウェアの更新だけで安全に使えるようにした『応急処置的』な規格」** です。


### 1. なぜTKIPが必要だったのか？

初期のWi-Fi暗号化規格である **WEP** には、致命的な弱点がありました。

* **WEPの弱点:** 暗号化に使う「鍵」が固定されており、データの塊（パケット）を一定数解析されると、簡単にパスワードが破られてしまうという問題です。

2000年代初頭、より強力な次世代規格（後のWPA2 / AES）が完成するまでには時間がかかる状況でした。そこで、既存の機器（WEP用ハードウェア）をそのまま使いつつ、セキュリティ強度を上げるために開発されたのがTKIPです。


### 2. TKIPが導入した3つの工夫

TKIPは、WEPと同じ暗号アルゴリズム（RC4）をベースにしながら、以下の仕組みを付け加えることで強度を高めました。

#### ① 一時鍵（Temporal Key）の採用

名前の由来通り、 **「暗号鍵を一定時間や一定の送信量ごとに自動的に更新する」** 仕組みです。これにより、万が一ひとつの鍵が解析されても、すぐに新しい鍵に切り替わるため、解読が非常に困難になりました。

#### ② 送信元MACアドレスの組み込み

鍵の生成に送信元のMACアドレスを混ぜることで、パケットごとに異なる鍵（キーストリーム）を生成し、WEPの弱点だった「同じパケットパターンの繰り返し」を防ぎました。

#### ③ MIC（Message Integrity Check）

データが途中で改ざんされていないかを確認するためのチェック機能（ハッシュ関数の一種）を追加しました。


### 3. 現在の立ち位置：TKIPは「使用禁止」

かつては救世主だったTKIPですが、現在は **「推奨されない（非推奨）」** 、あるいは **「使用禁止」** の規格となっています。

* **脆弱性の発見:** 2008年頃にTKIP特有の脆弱性が発見され、短時間で解読される可能性が示されました。
* **速度の制限:** IEEE 802.11n（Wi-Fi 4）以降の規格では、TKIPを使用すると通信速度が最大**54Mbps**（古いWi-Fi規格の速度）に制限されるというルールがあります。
* **AESへの移行:** 現在は、より強力でハードウェア処理による高速化が可能な **AES（WPA2 / WPA3の標準）** を使うのが当たり前となっています。
...
| 規格 | 暗号化アルゴリズム | 特徴 |
| --- | --- | --- |
| **WEP** | RC4 (固定鍵) | **【危険】** 数分で解読可能 |
| **WPA (TKIP)** | **RC4 (動的鍵)** | **【現在非推奨】** 応急処置版、速度低下の原因 |
| **WPA2 (AES)** | **AES** | **【現在の標準】** 強力で高速 |
| **WPA3 (SAE)** | **AES / GCMP** | **【最新】** さらに強固で安全 |


### まとめ

TKIPは、 **「古い機器を見捨てずに、ソフトウェアの知恵（鍵の自動更新）でWi-Fiを延命させた歴史的なプロトコル」** です。

しかし、現代の高速・安全なインターネット環境においては、すでに役目を終えています。もし自宅のWi-Fiルーターの設定で「TKIP」や「WPA-PSK (TKIP)」が選ばれていたら、安全と速度のために「AES」または「WPA2/WPA3」へ変更することをおすすめします。
